...
const TeamMember = ({ name, role, imageUrl }) => (
  ...
  >
    <img
      className="mx-auto h-24 w-24 rounded-full object-cover shadow-lg border-4 border-white"
      src={imageUrl}
      alt={name}
    />
    <h3 className="mt-4 text-lg font-semibold">{name}</h3>
    <p className="text-sm text-sky-600">{role}</p>
  </motion.div>
);
    ...
    <div className="container mx-auto">
      ...
      >
        <img
          ...
          className="h-20 w-20 mx-auto mb-4 rounded-full shadow-lg border-2 border-sky-400"
        />
        <h1 className="text-4xl md:text-5xl font-extrabold text-sky-800 drop-shadow">
          We are <span className="text-blue-600">SharedSpoon</span>
        </h1>
      ...
      <div className="my-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          <div className="text-center p-6 bg-white rounded-xl shadow-lg">
            <UsersIcon className="h-12 w-12 text-sky-500 mx-auto mb-4" />
            <h3 className="font-bold text-xl text-gray-800 mb-2">Community First</h3>
            <p className="text-gray-600">We empower local communities to support each other, fostering connection and reducing hunger together.</p>
          </div>
          <div className="text-center p-6 bg-white rounded-xl shadow-lg">
            <HeartIcon className="h-12 w-12 text-pink-500 mx-auto mb-4" />
            <h3 className="font-bold text-xl text-gray-800 mb-2">Act with Compassion</h3>
            <p className="text-gray-600">Every action is driven by empathy and a genuine desire to help our neighbors and our planet.</p>
          </div>
          <div className="text-center p-6 bg-white rounded-xl shadow-lg">
            <GlobeAltIcon className="h-12 w-12 text-green-500 mx-auto mb-4" />
            <h3 className="font-bold text-xl text-gray-800 mb-2">Drive Sustainability</h3>
            <p className="text-gray-600">We are committed to creating a greener world by ensuring that good food nourishes people, not landfills.</p>
          </div>
        </div>
      </div>

      
      <div className="text-center bg-sky-600 text-white p-12 my-20 rounded-2xl shadow-xl">
        ...
        <a
          ...
          className="inline-block px-8 py-3 rounded-lg bg-white text-sky-600 font-semibold shadow-md hover:bg-sky-100 transition"
        >
          Get Started
        </a>
      </div>
